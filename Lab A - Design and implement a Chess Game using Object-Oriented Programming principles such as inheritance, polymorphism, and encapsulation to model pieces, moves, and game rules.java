...
class Board {
    ...
    private void setup() {
        ...
        for (int x = 0; x < 8; x++) {
            place(x, 0, makePiece(order[x], true));   
            place(x, 1, new Pawn(true));              
            place(x, 6, new Pawn(false));              
            place(x, 7, makePiece(order[x], false));
        }
    }
            ...
            for (int x1 = 0; x1 < 8; x1++) {
                ...
                for (int y2 = 0; y2 < 8; y2++)
                    for (int x2 = 0; x2 < 8; x2++) {
                        ...
                        end.setPiece(p); start.setPiece(null);          
                        boolean stillInCheck = isInCheck(white);
                        start.setPiece(p); end.setPiece(captured);     
